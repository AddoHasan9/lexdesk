...
// sanitize a case object — remove non-serializable values before save
function sanitizeCase(c){
  ...
  for(const k of allowed){
    ...
    // deep-copy log/comments arrays safely
    if(k === 'log' || k === 'comments'){
...
async function sbSave(){
  try {
    if(cases.length > 0){
      ...
      if(!r.ok){ console.error('sbSave upsert failed', r.status); throw new Error('HTTP '+r.status); }
    }
    // Only delete cases that were explicitly deleted in THIS session
    if(_pendingDeletes.length > 0){
      const ids = _pendingDeletes.join(',');
      await sbFetch(SB_URL+'/rest/v1/cases?id=in.('+ids+')',{
...
async function sbDeleteCase(id){
  try {
    await sbFetch(SB_URL+'/rest/v1/cases?id=eq.'+id,{method:'DELETE',headers:SB_H});
  } catch(e){ console.warn('sbDeleteCase failed (offline?)', e); }
}

// ══ SUPABASE META (reminders + notifications) ══
// لازم تنشئ الجدول من SQL Editor في Supabase:
// CREATE TABLE IF NOT EXISTS meta (id text PRIMARY KEY, data jsonb, updated_at timestamptz);
// ALTER TABLE meta ENABLE ROW LEVEL SECURITY;
// CREATE POLICY "allow_all" ON meta FOR ALL TO anon USING (true) WITH CHECK (true);
async function sbSaveMeta(key, data){
  ...
  }catch(e){ console.warn('sbSaveMeta failed',e); }
}
...
async function saveData(){
  ...
  saveTimer=setTimeout(async()=>{
    try{
      ...
      // retry once more after 5s silently
      setTimeout(async()=>{
...
function applyRoleUI(){
  ...
  const sbSet=document.getElementById('sbSettings');
  if(sbSet)sbSet.style.display=isAdmin()?'flex':'none';
  const mnSet=document.getElementById('mnSet');
  if(mnSet)mnSet.style.display=isAdmin()?'flex':'none';
  // Reports tab — admin only
  const sbReports=document.getElementById('sbReports');
  if(sbReports)sbReports.style.display=isAdmin()?'flex':'none';
  const mnReports=document.getElementById('mnReports');
  if(mnReports)mnReports.style.display=isAdmin()?'flex':'none';
  // Export Excel & Print — admin only
  ...
  if(av)av.textContent=isAdmin()?'م':'ع';
  const pillSpan=document.querySelector('.user-pill span');
  if(pillSpan)pillSpan.textContent=isAdmin()?ADMIN_USER:'مستخدم';
}
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  body.addEventListener('click', e=>{
    // ignore clicks on action buttons, status/stage chips, links
...
function render(){
  ...
  const fil=cases.filter(c=>{
    ...
    if(fl&&c.lawyer!==fl)return false;
...
function renderCard(c){
  ...
  return`<div class="case-card">
    <div class="cc-clickarea" onclick="openDetail(${c.id})" style="cursor:pointer">
      <div class="cc-top">
        <div>
          <div class="cc-name">${c.company}</div>
          <div style="margin-top:4px"><span class="type-chip">${c.type}</span></div>
        </div>
        <span class="status-badge ${statusClass(c.status)}" onclick="event.stopPropagation();openStatusDrop(event,${c.id})">${c.status}</span>
      </div>
      <div class="cc-meta">
        <div style="display:flex;align-items:center;gap:5px;font-size:11px;font-weight:600">
          <div class="lawyer-dot" style="background:${lc}"></div>${c.lawyer}
        </div>
      </div>
      <div class="cc-amt">${amt}</div>
    </div>
    <div class="cc-foot">
      ...
      <div style="display:flex;gap:4px">
        ...
        <button class="act-btn del" title="حذف" onclick="askDel(${c.id})">
          <svg class="del-svg" viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>
          <span class="del-label">حذف</span>
        </button>
      </div>
    </div>
  </div>`;
...
function buildCharts(){
  const isDark=document.documentElement.getAttribute('data-theme')!=='light';
  const textColor=isDark?'rgba(255,255,255,.5)':'rgba(0,0,0,.4)';
  Chart.defaults.color=textColor;
  Chart.defaults.borderColor=isDark?'rgba(255,255,255,.07)':'rgba(0,0,0,.07)';
  // status donut
  const statuses=['قيد المعالجة','منجزة','معلقة','مراجعة','ناقصة'];
  const sCounts=statuses.map(s=>cases.filter(c=>c.status===s).length);
  buildChart('chartStatus','doughnut',statuses,sCounts,['#F0A500','#00C48C','#FF4D6A','#F0A500','#9B6DFF']);
  // lawyers bar
  const lNames=settings.lawyers;
  const lCounts=lNames.map(l=>cases.filter(c=>c.lawyer===l).length);
  buildChart('chartLawyers','bar',lNames,lCounts,LAWYER_COLORS);
  // types
  const typesMap={};cases.forEach(c=>{typesMap[c.type]=(typesMap[c.type]||0)+1;});
  const tKeys=Object.keys(typesMap).sort((a,b)=>typesMap[b]-typesMap[a]).slice(0,6);
  buildChart('chartTypes','bar',tKeys,tKeys.map(k=>typesMap[k]),['#3B7EFF','#00C48C','#F0A500','#FF4D6A','#9B6DFF','#FF8C42']);
  // amounts
  const amts=settings.lawyers.map(l=>cases.filter(c=>c.lawyer===l).reduce((s,c)=>s+(c.amountIQD||0),0));
  buildChart('chartAmounts','bar',lNames,amts,LAWYER_COLORS);
}
...
function openStatusDrop(e,id){
  ...
  d.style.top=(r.bottom+4)+'px';d.style.right=(window.innerWidth-r.right)+'px';
}
function setStatus(id,val){
  const c=cases.find(x=>x.id===id);if(!c)return;
  const old=c.status;c.status=val;
...
function openForm(id){
  editingId=id;
  document.getElementById('formTitle').textContent=id?'تعديل المعاملة':'معاملة جديدة';
  ...
  document.getElementById('fDef').value=c.deficiency||'';
  document.getElementById('fNotes').value=c.notes||'';
  // reset attach
  pendingAttachFile=null;
  pendingAttachUrl=c.attachUrl||'';
  ...
  if(c.attachUrl){
    ...
    cur.innerHTML='';
    document.getElementById('attachZone').style.display='block';
  }
  ...
  selLawyer=c.lawyer||'';
  selStatus=c.status||'قيد المعالجة';
  selCur=c.currency||settings.defCurrency||'IQD';
  selDept=c.stage||'';
  buildLawyerSel();buildStatusOpts();buildDeptOpts();setCur(selCur);
  populateFormTypes();
  ...
  // wadea checklist
  const isWadea=(c.type||'')=== WADEA_TYPE;
  document.getElementById('wadeaRow').style.display=isWadea?'block':'none';
  setWadeaChecks(isWadea?(c.wadeaChecks||''):'');
  openOverlay('formOverlay');
}
function buildLawyerSel(){
  const el=document.getElementById('lawyerSel');
  el.innerHTML=settings.lawyers.map((l,i)=>`<div class="lawyer-opt${l===selLawyer?' sel':''}" onclick="pickLawyer('${l}')"><div style="width:10px;height:10px;border-radius:50%;background:${LAWYER_COLORS[i%LAWYER_COLORS.length]}"></div>${l}</div>`).join('');
}
...
function buildStatusOpts(){
  ...
  document.getElementById('statusOpts').innerHTML=opts.map(o=>{
    ...
    const style = sel
      ? `--sel-c:${o.c};--sel-bg:${o.bg};border-color:${o.c};background:${o.bg};color:${o.c};box-shadow:0 0 0 1px ${o.c} inset;`
      : '';
    return `<div class="status-opt${sel?' sel':''}" style="${style}" onclick="pickStatus('${o.v}','${o.c}','${o.bg}')">${o.icon} ${o.v}</div>`;
  }).join('');
}
function pickStatus(v,c,bg){selStatus=v;document.getElementById('holdRow').style.display=v==='معلقة'?'block':'none';document.getElementById('stageRow').style.display=(v!=='منجزة'&&v!=='معلقة')?'block':'none';buildStatusOpts();}
function buildDeptOpts(){
  const opts=[...settings.depts,'أخرى'];
  document.getElementById('deptOpts').innerHTML=opts.map(o=>`<div class="dept-opt${o===selDept||(!selDept&&o===opts[0])?' sel':''}" onclick="pickDept('${o}')">${o}</div>`).join('');
  document.getElementById('fStageOther').style.display=selDept==='أخرى'?'block':'none';
}
...
function populateFormTypes(){
  const s=document.getElementById('fType');
  const v=s.value;
  s.innerHTML='<option value="">— اختر النوع</option>'+settings.types.map(t=>`<option>${t}</option>`).join('');
  s.value=v;
  s.onchange=onTypeChange;
}
...
function onTypeChange(){
  ...
  const row=document.getElementById('wadeaRow');
  row.style.display=(t===WADEA_TYPE)?'block':'none';
}

function toggleWadea(n){
  const cb=document.getElementById('wc'+n);
  cb.checked=!cb.checked;
  document.getElementById('wcheck'+n).classList.toggle('checked',cb.checked);
}

function setWadeaChecks(val){
  // val = comma-separated names of checked items
  const checked=(val||'').split(',').map(s=>s.trim()).filter(Boolean);
  WADEA_ITEMS.forEach((item,i)=>{
    const n=i+1;
    const cb=document.getElementById('wc'+n);
    const lbl=document.getElementById('wcheck'+n);
    if(!cb||!lbl)return;
...
function getWadeaValue(){
  return WADEA_ITEMS.filter((_,i)=>{
    const cb=document.getElementById('wc'+(i+1));
...
async function saveCase(){
  ...
  if(editingId){const idx=cases.findIndex(c=>c.id===editingId);if(idx!==-1){const old=cases[idx];
    // audit log entry
    const logEntry={id:Date.now(),type:'edit',msg:'تم تعديل المعاملة',user:currentUser||'الأدمن',time:new Date().toISOString()};
...
function askDel(id){
  ...
  document.getElementById('confirmBtn').onclick=async()=>{
    ...
    // 2. Delete from Supabase DB immediately (not batch)
    sbDeleteCase(id);
    // 3. Delete attachment from Storage if exists
    if(c.attachUrl){
      try{
        const path = c.attachUrl.split('/object/public/'+SB_BUCKET+'/')[1];
        if(path){
  ...
  document.getElementById('confirmBtn').onclick=async()=>{
    ...
    for(const c of cases){
      if(c.attachUrl){
        try{
          const path=c.attachUrl.split('/object/public/'+SB_BUCKET+'/')[1];
          if(path) await fetch(SB_URL+'/storage/v1/object/'+SB_BUCKET+'/'+path,{
            method:'DELETE',
            headers:{'apikey':SB_KEY,'Authorization':'Bearer '+SB_KEY}
          });
        }catch(e){}
    ...
    // Delete all from DB
    try{ await fetch(SB_URL+'/rest/v1/cases',{method:'DELETE',headers:{...SB_H,'Prefer':'return=minimal'}}); }catch(e){}
    cases=[];
    localStorage.setItem(SK_D, JSON.stringify(cases));
    render();
    closeOverlay('confirmOverlay');
...
function goPage(p){
  ...
  pages.forEach(x=>{
    ...
    if(pg)pg.classList.toggle('active',x===p);
    // sidebar buttons
    const sbMap={dash:'sbDash',charts:'sbCharts',reports:'sbReports',settings:'sbSettings'};
    const sb=document.getElementById(sbMap[x]);
    if(sb)sb.classList.toggle('active',x===p);
  });
  if(p==='settings')loadSettingsPage();
  if(p==='charts'){document.getElementById('pageCharts').classList.add('active');setTimeout(buildCharts,100);}
...
function focusSearch(){document.getElementById('searchInp').focus();}

// ══ SETTINGS ══
function switchSetTab(tab){
  document.querySelectorAll('.set-tab').forEach(t=>t.classList.toggle('active',t.dataset.tab===tab));
  document.querySelectorAll('.set-tab-content').forEach(c=>{c.classList.remove('active');});
...
function loadSettingsPage(){
  ...
  renderTags('typeTags',settings.types,'type');
  renderTags('deptTags',settings.depts,'dept');
...
function populateAllDropdowns(){
  const ft=document.getElementById('filterType');const fl=document.getElementById('filterLawyer');
  const vt=ft.value,vl=fl.value;
  ft.innerHTML='<option value="">كل الأنواع</option>'+settings.types.map(t=>`<option>${t}</option>`).join('');
  fl.innerHTML='<option value="">كل المحامين</option>'+settings.lawyers.map(l=>`<option>${l}</option>`).join('');
  ft.value=vt;fl.value=vl;
  populateMobFilters();
}
...
async function uploadAttachment(file, company) {
  try {
    ...
    // Supabase Storage only accepts ASCII filenames
    const safeName = 'case_' + (Date.now()%100000);
    const fileName = Date.now() + '_' + safeName + '.' + ext;

    // Storage headers — no Content-Type in base headers for file upload
...
function removeAttach() {
  pendingAttachUrl = '';
  document.getElementById('attachCurrent').style.display = 'none';
  document.getElementById('attachZone').style.display = 'block';
}
...
(function(){
  ...
  function draw(){
    ...
    const color = isDark() ? '240,165,0' : '11,29,58';
    particles.forEach(p=>{
      ctx.beginPath();
...
const SFX = {
  ...
  _tone(freq, dur, vol=0.04, type='sine', fadeIn=0.01) {
    try {
      const ctx = this._get();
  ...
  play(type) {
    try {
      const ctx = this._get();
      if (type === 'add') {
        // نغمتان متتاليتان ناعمتان — مثل xylophone
        this._tone(880, 0.25, 0.04, 'sine', 0.005);
        setTimeout(() => this._tone(1108, 0.3, 0.03, 'sine', 0.005), 100);
      } else if (type === 'save') {
        // نقرة واحدة ناعمة خفيفة جداً
        this._tone(1046, 0.18, 0.03, 'sine', 0.004);
      } else if (type === 'delete') {
        // نغمة هابطة ناعمة
        try {
          const c = this._get();
          ...
          g.gain.linearRampToValueAtTime(0.04, c.currentTime + 0.01);
        ...
        // ping ناعم واحد فقط
        this._tone(1318, 0.4, 0.03, 'sine', 0.005);
      } else if (type === 'login') {
        // ثلاث نغمات صاعدة خفيفة — مثل chime
        this._tone(523, 0.3, 0.03, 'sine', 0.008);
        setTimeout(() => this._tone(659, 0.3, 0.03, 'sine', 0.008), 120);
        setTimeout(() => this._tone(784, 0.45, 0.04, 'sine', 0.008), 240);
      }
    } catch(e) {}
  }
};
...
function countUp(el, target, prefix, suffix, duration) {
  if (!el) return;
  // always show final value immediately if 0
  if (target === 0) {
    el.textContent = (prefix||'') + '0' + (suffix||'');
    try { el.classList.remove('stat-pop'); el.offsetHeight; el.classList.add('stat-pop'); } catch(e){}
    return;
  }
...
function openDetail(id){
  try{
    id = Number(id);
    const c = cases.find(x=>x.id===id);
    if(!c){ console.warn('openDetail: case not found', id); return; }
    detailCaseId = id;
    const lci = settings.lawyers.indexOf(c.lawyer);
    const lc = LAWYER_COLORS[lci%LAWYER_COLORS.length];
    document.getElementById('detAvatar').textContent = (c.company||'؟')[0];
    document.getElementById('detAvatar').style.background = `linear-gradient(135deg,${lc},${lc}bb)`;
    document.getElementById('detName').textContent = c.company||'—';
    document.getElementById('detType').textContent = c.type||'—';
    ...
    document.getElementById('detDefRow').style.display = c.deficiency?'block':'none';
    document.getElementById('detDef').textContent = c.deficiency||'—';
    ...
    if(wadeaDetailRow){
      if(c.type===WADEA_TYPE){
        wadeaDetailRow.style.display='block';
        const checked=(c.wadeaChecks||'').split('،').map(s=>s.trim()).filter(Boolean);
      ...
      } else { wadeaDetailRow.style.display='none'; }
    }
    renderDetailComments(c);
...
function renderDetailComments(c){
  ...
  el.innerHTML = comments.map(cm=>{
    ...
    const ts = t.toLocaleDateString('ar-IQ',{month:'short',day:'numeric'})+' '+t.toLocaleTimeString('ar',{hour:'2-digit',minute:'2-digit'});
    return `<div class="comment-item"><button class="comment-del" onclick="deleteComment(${cm.id})" title="حذف">✕</button><div class="comment-txt">${cm.text}</div><div class="comment-meta"><span style="font-weight:700;color:var(--text)">${cm.user}</span><span>•</span><span>${ts}</span></div></div>`;
  }).join('');
}
...
function addComment(){
  ...
  if(!c)return;
  const cm = {id:Date.now(),text,user:currentUser||'الأدمن',time:new Date().toISOString()};
  if(!c.comments)c.comments=[];
  c.comments.push(cm);
  if(!c.log)c.log=[];
  c.log.push({id:Date.now()+1,type:'comment',msg:'تم إضافة تعليق',user:currentUser||'الأدمن',time:new Date().toISOString()});
  saveData();
  renderDetailComments(c);
  renderDetailTimeline(c);
  document.getElementById('commentInp').value='';
...
function deleteComment(cid){
  ...
  c.comments = c.comments.filter(x=>x.id!==cid);
  saveData();
  renderDetailComments(c);
}
function detailEdit(){closeDetail();openForm(detailCaseId);}
...
function getRepCases(){
  const now = new Date();
  return cases.filter(c=>{
    if(repPeriod==='all')return true;
    if(!c.date&&!c.addedAt)return repPeriod==='all';
    const d = new Date(c.date||c.addedAt);
    if(repPeriod==='year')return d.getFullYear()===now.getFullYear();
    if(repPeriod==='month')return d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear();
    return true;
  });
}
function buildReports(){
  const filtered = getRepCases();
  const totalIQD = filtered.reduce((s,c)=>s+(c.amountIQD||0),0);
  const totalUSD = filtered.reduce((s,c)=>s+(c.amountUSD||0),0);
  const done = filtered.filter(c=>c.status==='منجزة').length;
  // summary cards
  ...
  const months={};
  filtered.forEach(c=>{
    const d=new Date(c.date||c.addedAt||Date.now());
    const key=d.getFullYear()+'-'+(d.getMonth()+1).toString().padStart(2,'0');
    const lbl=d.toLocaleDateString('ar-IQ',{month:'short',year:'numeric'});
    if(!months[key])months[key]={lbl,iqd:0,usd:0,count:0};
    months[key].iqd+=(c.amountIQD||0);
    months[key].usd+=(c.amountUSD||0);
    months[key].count++;
  });
    ...
    repM.innerHTML=sortedMonths.map(([,v])=>`
      <div class="month-row">
        ...
        <div class="month-bar-wrap">
          <div class="month-bar-fill" style="background:linear-gradient(90deg,var(--gold),var(--gold2));width:${Math.max(v.iqd/maxIQD*100,3)}%">
            ${v.iqd>0?`<span>${v.count} معاملة</span>`:''}
          </div>
        </div>
        <div class="month-amt">${fmt(v.iqd)} د.ع</div>
        <div class="month-count">${v.usd>0?'$'+fmt(v.usd):''}</div>
      </div>`).join('');
  }
  // lawyers
  const repL=document.getElementById('repLawyers');
  if(repL){
    repL.innerHTML=settings.lawyers.map((l,i)=>{
      const lCases=filtered.filter(c=>c.lawyer===l);
      const lIQD=lCases.reduce((s,c)=>s+(c.amountIQD||0),0);
      const lUSD=lCases.reduce((s,c)=>s+(c.amountUSD||0),0);
      const lDone=lCases.filter(c=>c.status==='منجزة').length;
      const col=LAWYER_COLORS[i%LAWYER_COLORS.length];
      return`<div class="lawyer-report-row">
        <div class="lr-av" style="background:${col}">${l[0]}</div>
        <div class="lr-info">
          <div class="lr-name">${l}</div>
          <div class="lr-stats">
            <div class="lr-stat">معاملات: <span>${lCases.length}</span></div>
            <div class="lr-stat">منجزة: <span>${lDone}</span></div>
          </div>
        </div>
        <div class="lr-amts">
          <div class="lr-iqd">${fmt(lIQD)} د.ع</div>
          ${lUSD>0?`<div class="lr-usd">$${fmt(lUSD)}</div>`:''}
        </div>
      </div>`;
    }).join('');
  }
  // statuses
  const repS=document.getElementById('repStatuses');
  if(repS){
    const sList=['قيد المعالجة','منجزة','معلقة','مراجعة','ناقصة'];
    const sColors={'قيد المعالجة':'var(--green)','منجزة':'var(--blue2)','معلقة':'var(--red)','مراجعة':'var(--gold)','ناقصة':'var(--purple)'};
    repS.innerHTML=sList.map(s=>{
      const n=filtered.filter(c=>c.status===s).length;
      const pct=filtered.length?Math.round(n/filtered.length*100):0;
      return`<div class="rep-summary-row">
  ...
  // top clients
  const repT=document.getElementById('repTopClients');
  if(repT){
    ...
    repT.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:10px">`+sorted.map((c,i)=>{
      ...
      return`<div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:14px">
        ...
        <div style="font-size:14px;font-weight:700;margin-bottom:4px">${c.company}</div>
        <div style="font-size:11px;color:var(--text2);margin-bottom:8px">${c.type}</div>
        <div style="height:4px;background:var(--border);border-radius:2px;overflow:hidden;margin-bottom:8px"><div style="height:100%;width:${pct}%;background:linear-gradient(90deg,var(--gold),var(--gold2));border-radius:2px"></div></div>
        <div style="font-family:'Cairo';font-size:16px;font-weight:900;color:var(--gold)">${fmt(c.amountIQD)} د.ع</div>
      </div>`;
    }).join('')+'</div>';
  }
}
...
// ══ MOBILE FILTERS SYNC ══
function syncMobFilter(type,val){
  if(type==='type')document.getElementById('filterType').value=val;
  else if(type==='lawyer')document.getElementById('filterLawyer').value=val;
  else if(type==='status')document.getElementById('filterStatus').value=val;
  render();
}
function populateMobFilters(){
  const mt=document.getElementById('mobFilterType');
  const ml=document.getElementById('mobFilterLawyer');
  if(!mt||!ml)return;
  const vt=mt.value,vl=ml.value;
  mt.innerHTML='<option value="">كل الأنواع</option>'+settings.types.map(t=>`<option>${t}</option>`).join('');
  ml.innerHTML='<option value="">كل المحامين</option>'+settings.lawyers.map(l=>`<option>${l}</option>`).join('');
  mt.value=vt;ml.value=vl;
}

// ══ REMINDERS ══
const SK_REM = 'lexdesk_reminders';
let reminders = [];
let selRemCat = 'عام';
...
async function loadReminders() {
  const cloud = await sbLoadMeta('reminders');
  if(cloud && Array.isArray(cloud)){ reminders=cloud; }
  else { try{ reminders=JSON.parse(localStorage.getItem(SK_REM)||'[]'); }catch(e){ reminders=[]; } }
  renderRemBadge(); renderRemList();
}
function saveReminders() {
  try{localStorage.setItem(SK_REM, JSON.stringify(reminders));}catch(e){}
  sbSaveMeta('reminders', reminders);
}
function renderRemBadge() {
  const active = reminders.filter(r => !r.done).length;
  const b = document.getElementById('remBadge');
  if (!b) return;
  b.textContent = active > 9 ? '9+' : active;
  b.classList.toggle('show', active > 0);
}
...
function toggleRemPanel() {
  const p = document.getElementById('remPanel');
  const isOpen = p.classList.contains('open');
  // close other panels first
  document.getElementById('notifPanel').classList.remove('open');
  p.classList.toggle('open', !isOpen);
  if (!isOpen) renderRemList();
}
function openRemForm() {
  document.getElementById('remForm').style.display = 'block';
  document.getElementById('remText').focus();
}
function closeRemForm() {
  document.getElementById('remForm').style.display = 'none';
  document.getElementById('remText').value = '';
  document.getElementById('remDate').value = '';
}
function pickRemCat(el) {
  document.querySelectorAll('.rem-cat-opt').forEach(o => {
  ...
  const c = REM_CATS[el.dataset.cat] || REM_CATS['عام'];
  el.style.background = c.bg;
  el.style.color = c.color;
  el.style.borderColor = c.color.replace('var(','').replace(')','');
  selRemCat = el.dataset.cat;
  selRemColor = c.color;
}
function saveReminder() {
  const text = (document.getElementById('remText').value || '').trim();
  if (!text) { document.getElementById('remText').style.borderColor = 'var(--red)'; return; }
  document.getElementById('remText').style.borderColor = '';
  const r = {
    id: Date.now(),
    text,
    cat: selRemCat,
