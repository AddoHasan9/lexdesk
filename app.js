...
// آمنة للاستخدام داخل onclick="...(‘…’)" — تهرّب أولاً كسلسلة JS (backslash/quote) ثم كسمة HTML (fixes real stored-XSS risk via free-text fields like company name)
...
function renderRow(c){
  ...
  const stg=c.stage?'<span class="stage-chip" onclick="openStageDrop(event,'+c.id+')">'+esc(c.stage)+'</span>':'<span class="stage-chip" onclick="openStageDrop(event,'+c.id+')" style="color:var(--text3)">—</span>';
  let statusHtml='<span class="status-badge '+statusClass(c.status)+'" onclick="openStatusDrop(event,'+c.id+')">'+esc(c.status)+'</span>';
